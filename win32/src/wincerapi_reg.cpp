...
PyObject *PyWinObject_FromCEHKEY(HKEY h)
{
	return new PyCEHKEY(h);
}
...
// @pymethod (<o PyHKEY>, int)|wincerapi|CeRegCreateKeyEx|Creates the specified key, or opens the key if it already exists.
PyObject *
PyCeRegCreateKeyEx( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	PyObject *obSubKey;
...
PyObject *
PyCeRegDeleteKey( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	PyObject *obSubKey;
...
PyObject *
PyCeRegDeleteValue( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	PyObject *obSubKey;
	...
	PyW32_BEGIN_ALLOW_THREADS
	rc=CeRegDeleteValue(hKey, subKey);
	PyW32_END_ALLOW_THREADS
	PyWinObject_FreeTCHAR(subKey);
	if (rc!=ERROR_SUCCESS)
...
// @pymethod (string, string)|wincerapi|CeRegEnumKeyEx|Enumerates subkeys of the specified open registry key. The function retrieves the name of one subkey each time it is called.
PyObject *
PyCeRegEnumKeyEx( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	int index;
...
static int
countStrings(TCHAR *data, int len)
{
	int strings;
	TCHAR *P;
	TCHAR *Q = data + len;
	...
	switch (typ) {
		...
		case REG_EXPAND_SZ: {
			...
			*retDataBuf=(BYTE *)PyMem_NEW(DWORD, *retDataSize);
			...
			if (temp) PyWinObject_FreeTCHAR(temp);
			}
			break;
		case REG_MULTI_SZ:
			{
...
// @pymethod (string,object,type)|wincerapi|CeRegEnumValue|Enumerates values of the specified open registry key. The function retrieves the name of one subkey each time it is called.
PyObject *
PyCeRegEnumValue( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	int index;
	...
	if ((rc=CeRegQueryInfoKey( hKey, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
		             &retValueSize, NULL, NULL, NULL))!=ERROR_SUCCESS) {
	...
	{
	PyW32_BEGIN_ALLOW_THREADS
	rc=CeRegEnumValue(hKey, index, retValueBuf, &retValueSize, NULL, &typ, (BYTE *)retDataBuf, &retDataSize);
	PyW32_END_ALLOW_THREADS
	}
	if (rc!=ERROR_SUCCESS) {
...
PyObject *
PyCeRegOpenKeyEx( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	PyObject *obSubKey;
	...
	PyW32_BEGIN_ALLOW_THREADS
	rc=CeRegOpenKeyEx(hKey, subKey, res, sam, &retKey);
	PyW32_END_ALLOW_THREADS
	PyWinObject_FreeTCHAR(subKey);
	if (rc!=ERROR_SUCCESS)
...
PyObject *
PyCeRegQueryInfoKey( PyObject *self, PyObject *args)
{
  HKEY hKey;
  PyObject *obKey;
  long rc;
  ...
  if ((rc=CeRegQueryInfoKey(hKey, NULL, NULL, 0, &nSubKeys, NULL, NULL, &nValues,
    NULL,
    NULL,
    NULL,
    &ft)
...
PyObject *
PyCeRegSetValueEx( PyObject *self, PyObject *args )
{
	HKEY hKey;
	PyObject *obKey;
	PyObject *obValueName;
	...
	PyW32_BEGIN_ALLOW_THREADS
	rc=CeRegSetValueEx(hKey, valueName, NULL, typ, data, len );
	PyW32_END_ALLOW_THREADS
	PyWinObject_FreeTCHAR(valueName);
	if (rc!=ERROR_SUCCESS)
